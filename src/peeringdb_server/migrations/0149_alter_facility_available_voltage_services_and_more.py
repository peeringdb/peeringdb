# Generated by Django 5.2.12 on 2026-03-10 16:44

import django_peeringdb.fields
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('peeringdb_server', '0148_merge_20260310_1550'),
    ]

    operations = [
        migrations.AlterField(
            model_name='facility',
            name='available_voltage_services',
            field=django_peeringdb.fields.MultipleChoiceField(blank=True, choices=[('No Power', 'No Power'), ('48 VDC', '48 VDC'), ('400 VAC', '400 VAC'), ('480 VAC', '480 VAC')], help_text='The alternating current voltage available to users of the facility either directly from the landlord or delivered by the utility separately.', max_length=255, null=True, verbose_name='Available Voltage Services'),
        ),
        migrations.AlterField(
            model_name='oauthapplication',
            name='authorization_grant_type',
            field=models.CharField(choices=[('authorization-code', 'Authorization code'), ('urn:ietf:params:oauth:grant-type:device_code', 'Device Code'), ('implicit', 'Implicit'), ('password', 'Resource owner password-based'), ('client-credentials', 'Client credentials'), ('openid-hybrid', 'OpenID connect hybrid')], max_length=44),
        ),
    ]
