...
from peeringdb_server.models import User, EmailAddress
...
def profile_user():
    ...
    user = User.objects.create_user(username="user", password="user", email="user@localhost")
    user.set_verified()

    EmailAddress.objects.create(email="user@localhost", verified=True, user=user, primary=True)
...
def test_add_email(profile_user):
    ...
    response = client.post(reverse("profile-add-email"), {
    ...
    assert LogEntry.objects.filter(change_message="User added email address user@domain.com").exists()
...
def test_remove_email(profile_user):
    ...
    response = client.post(reverse("profile-remove-email"), {
        "email" : "user@domain.com",
    })
    ...
    assert LogEntry.objects.filter(change_message="User removed email address user@domain.com").exists()
...
def test_change_primary(profile_user):
    ...
    response = client.post(reverse("profile-set-primary-email"), {
        "email" : "user@domain.com",
    })
