...
def collect_exchanges():
    ...
    mapping = {}
    with open(cached_ix_path, "r") as file:
        data = json.load(file)
...
def collect_networks():
    ...
    mapping = {}
    with open(cached_net_path, "r") as file:
        data = json.load(file)
...
def collect_carriers():
    ...
    mapping = {}
    with open(cached_carrier_path, "r") as file:
        data = json.load(file)
...
def fac_export_kmz(limit=None):
    ...
    with open(cached_fac_path, "r") as file:
        data = json.load(file)
    ...
    for fac in data.get("data")[:limit]:
        if fac.get("latitude") and fac.get("longitude"):
            ...
            for key,value in fac.items():
                ...
                point.extendeddata.newdata(name=key, value=escape(value), displayname=key.title())
