...
- `rest.py`: handle django-rest-framework view set up and querying logic
- `rest_throttles.py`: custom rate limiting handlers
- `renderers.py`: handles rendering of the REST API response

## Serializers

Serializers are defined in `serializers.py` and should extend the custom `ModelSerializer` defined in there.
...
Requests that dont do any searches and are not accessing an object directly will make sure of api-cache files to render the response.

So for example `/api/net` will use the api-cache, `/api/net/1` or `/api/net?id=1` will not.
...
- Fields cannot be easily removed from the API once the field has gone live, a field will be there until the next MAJOR version release of the api.
  So great care should be given when determining to add new fields.
- Fields cannot be removed until the next MAJOR version release of the API. If a field needs to be deprecated it should remain in the response and
  be changed to be read-only and set to some pre-determined constant value.
...
When making changes to the API one should always check that the client sync against that version of the API is still functional with the current version of peeringdb-py
...
When writing mass data migrations one should ask themselves if this is an update that needs to be propagated to local peoples snapshots.
...
In some cases changes to API mean the client is no longer compatible.
